...
from hexapod_tethered_utils.tension_speed_controller import (
	TensionSpeedController,
	TensionSpeedControllerConfig,
)

from motor_igh_sdk.deploy_real_el4090_pysoem_spool_speed import (
	RL_Real_PySOEM_WithSpoolSpeed,
)
...
def _rpm_from_radps(rad_s: float) -> float:
...
def main() -> int:
	...
	tsc = TensionSpeedController(
		TensionSpeedControllerConfig(
			...
			speed_limit_rpm=float(cfg.tsc_speed_limit_rpm),
	...
	# Hardware init
	robot = RL_Real_PySOEM_WithSpoolSpeed(args.ifname)
	robot.spool_command_buffer.current_limit_01a[0] = int(args.spool_current_limit_01a)
