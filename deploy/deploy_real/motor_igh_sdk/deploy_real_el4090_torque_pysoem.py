...
class RL_Real_Speed_PySOEM:
    """(Deprecated) speed-mode wrapper.

    NOTE: This file has been converted to torque-mode control.
    Use `RL_Real_Torque_PySOEM` instead.
    """


class RL_Real_Torque_PySOEM:
    """EtherCAT 力矩模式控制（封装格式与 speed 版保持一致，只替换下发指令）。

    官方 C SDK 对应：`set_motor_cur_tor`，其中 ctrl_status=1 表示“力矩控制”。

    约定：
    - 单电机测试：policy index 长度为 1
    - 指令以 policy index 对齐：`motor_command_buffer.target_torque_nm`
    - 方向统一：命令与反馈均乘以 `motor_direction_`
    """
    ...
    def __init__(self, ifname: str, *, motor_id: int = 19):
        ...
        # Command/State buffers (match speed wrapper style)
        self.motor_command_buffer = type("CommandBuffer", (), {})()
        # Torque command in Nm. Official scaling is 0.01 Nm per LSB.
        self.motor_command_buffer.target_torque_nm = [0.0] * self.num_dofs
        # ctrl_status: 1=torque control (recommended), see official docs for brake modes.
        self.motor_command_buffer.ctrl_status = [1] * self.num_dofs
        # 0=no ack, 1~3=response frame type
    ...
    def HardwareSendTorque(self):
        """把 policy 的力矩指令映射到对应 motor_id，并按 EtherCAT passage 下发。"""
        for policy_idx in range(self.num_dofs):
            ...
            # 力矩方向统一：motor_direction 也应用于力矩命令。
            torque_nm = self.motor_direction_[motor_id] * float(self.motor_command_buffer.target_torque_nm[policy_idx])
            ctrl_status = int(self.motor_command_buffer.ctrl_status[policy_idx])
            ack = int(self.motor_command_buffer.ack_status[policy_idx])

            tx_msg = self.motorData.getTxMsg(slave_idx)
            if ctrl_status == 1:
                set_motor_torque(tx_msg, addr["passage"], motor_id, torque_nm=torque_nm, ack_status=ack)
            else:
                # For brake/current modes, use raw 0.01 unit scaling.
                cur_tor_001 = int(round(torque_nm * 100.0))
                set_motor_cur_tor(
                    tx_msg,
                    addr["passage"],
                    motor_id,
                    cur_tor_001=cur_tor_001,
    ...
    def pd_loop(self):
        print("Starting Process Data Loop (Torque Mode)...")
        self.running = True
...
def _find_policy_index_by_motor_id(robot: RL_Real_Torque_PySOEM, motor_id: int) -> int:
...
if __name__ == "__main__":
    # Quick packing self-test (no EtherCAT required):
    #   python deploy_real_el4090_torque_pysoem.py --pack-test <motor_id> <torque_nm>
    if len(sys.argv) > 1 and sys.argv[1] == "--pack-test":
        motor_id = int(sys.argv[2]) if len(sys.argv) > 2 else 19
        torque_nm = float(sys.argv[3]) if len(sys.argv) > 3 else 0.0
        msg = EtherCAT_Msg_Base()
        msg.motor_num = 6
        msg.can_ide = 1
        set_motor_torque(msg, 1, motor_id, torque_nm=torque_nm, ack_status=1)
        data = [int(msg.motor[0].data[i]) for i in range(8)]
        print(f"pack-test motor_id={motor_id} torque_nm={torque_nm:.3f} -> data={data}")
        sys.exit(0)
    ...
    # 可选: 第三个参数 指定恒定力矩指令（Nm）。默认 0，安全不动。
    const_torque_nm = float(sys.argv[3]) if len(sys.argv) > 3 else 0.0

    print(f"Launching Torque Mode on interface {IFNAME}, motor_id={motor_id}")

    robot = None
    try:
        robot = RL_Real_Torque_PySOEM(IFNAME, motor_id=motor_id)
        idx = _find_policy_index_by_motor_id(robot, motor_id)

        # 初始化：所有电机力矩为 0
        for i in range(robot.num_dofs):
            robot.motor_command_buffer.target_torque_nm[i] = 0.0
            robot.motor_command_buffer.ctrl_status[i] = 1
        ...
        while True:
            ...
            # 恒定力矩（Nm）
            robot.motor_command_buffer.target_torque_nm[idx] = const_torque_nm
            ...
            if int(t * 10) % 10 == 0:
                ...
                print(
                    f"t={t:6.2f}s cmd_torque(Nm)={const_torque_nm:8.3f} "
