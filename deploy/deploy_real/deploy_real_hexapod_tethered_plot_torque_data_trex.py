...
# Fixed physical parameters for the real-world TReX comparison.  The policy
# still outputs only the 18 leg-joint actions; tether tension is generated by
# the deterministic literature controller below.
...
# Keep the real controller on the same reachable tension interval and the
# same observation normalization used during training (cable ctrl = T / 200).
...
class Controller:
    def __init__(self,config:Config) -> None:
        ...
        self.counter = 0
        # Before the first policy cycle the spool is explicitly zeroed, so the
        # causally correct previous cable command is zero.  The first TReX
        # target is applied by run() and appears in obs[63] one cycle later.
    ...
    @staticmethod
    def _compute_trex_tension(local_vx: float) -> float:
        """Return the TReX reference tension for the fixed real test slope.

        The tether is fixed at zero in-plane yaw and is assumed parallel to
        the slope, as in the real experiment.  Therefore beta=0 and phi=0 in
        TReX Eqs. 4.11--4.12, giving

          lambda = clip(v_x / v_max, -1, 1)
          T = M g [sin(alpha) + lambda mu cos(alpha)].

        The 0.20 m remote-anchor height affects the out-of-plane cable angle,
        but the training controller deliberately projects the tether into the
        slope plane, so that height does not enter this comparison law.
        """
        if not np.isfinite(local_vx):
            # Never allow an invalid IMU sample to become a NaN motor command.
            local_vx = 0.0

        alpha = np.deg2rad(SLOPE_ANGLE_DEG)
        beta = np.deg2rad(TETHER_YAW_DEG)

        # With the robot walking uphill and the tether yaw fixed to zero, the
        # body-forward/tether bearing phi is also zero.
        cos_phi = np.cos(beta)
        lambda_f = np.clip(
            cos_phi * float(local_vx) / TREX_VMAX_M_S,
            -1.0,
            1.0,
        )
        ...
        friction_radicand = (
            cos_alpha**2 * FRICTION_COEFFICIENT**2
            - sin_alpha**2 * sin_beta_sq
        )
        friction_margin = np.sqrt(max(float(friction_radicand), 0.0))

        weight_n = ROBOT_MASS_KG * GRAVITY_M_S2
        target_n = weight_n * (
            sin_alpha * cos_beta + lambda_f * friction_margin
        )
    ...
    def run(self):
        ...
        if self.action.shape != (18,):
            raise ValueError(
                "TReX comparison policy must output exactly 18 leg actions, "
                f"got shape {self.action.shape}"
            )

        # The policy controls only the 18 leg joints.  TReX, not the policy,
        # supplies the cable target.
        ...
        target_tension = self._compute_trex_tension(local_vx=float(linvel[0]))
        ...
        # Apply the same direct feedforward conversion as the constant-force
        # baseline: spool torque tau = tension T * effective radius r.  No
        # tension-feedback/PID term is added to this deterministic comparison.
