...
# Fixed physical parameters for the real-world Nagatani comparison.  The
# policy still outputs only the 18 leg-joint actions; the cable target is
# generated by the deterministic literature controller below.
...
# Match the reachable tension interval and cable-control normalization used by
# training: the MuJoCo cable gain is -200 N/ctrl, hence positive tension has
# the observation-side magnitude T / 200.
...
class Controller:
    def __init__(self,config:Config) -> None:
        ...
        self.counter = 0
        # The spool is zeroed before the first policy cycle, so the causally
        # correct previous cable command and initial held target are both zero.
        self._tether_force_target_n = 0.0
        self._previous_tether_ctrl = 0.0
        self._tether_controller_feasible = False
        self._tether_geometry_valid = False
        # Initialize the lightweight MJCF-derived kinematic constants once.
        # Per-cycle calls only update the 18 joint angles; no simulator or
        # contact-state interface is used.
        self._tether_kinematics = HexapodMjcfKinematics()
    ...
    def _compute_nagatani_tension(
        self,
        gravity_body: np.ndarray,
        command: np.ndarray,
        gravity_valid: bool,
    ) -> tuple[float, bool, bool]:
        """Return Nagatani tension from dynamic all-six-foot geometry.

        This is the same contact-free controller used by the corresponding
        sim2sim script.  Every foot site participates in the support interval,
        regardless of gait phase.  The paper's planar model fixes theta_T=0,
        so the 0.20 m remote-anchor height is recorded above but does not enter
        the force law.  ``h_t`` is instead the dynamic height of the robot-side
        tether attachment above the six-foot support surface.

        If either IMU gravity or MJCF geometry is invalid, retain the preceding
        valid target.  The initialized target is zero, so an invalid first
        sample cannot create a nonzero spool command.
        """
        if not gravity_valid:
            return (
                float(self._tether_force_target_n),
                bool(self._tether_controller_feasible),
                False,
            )

        geometry = self._tether_kinematics.forward(
            self.qj,
            arm_yaw_rad=np.deg2rad(TETHER_YAW_DEG),
        )
        if not bool(geometry.geometry_valid):
            return (
                float(self._tether_force_target_n),
                bool(self._tether_controller_feasible),
                False,
        ...
        gravity_body = np.asarray(gravity_body, dtype=np.float64).reshape(3)
        command = np.asarray(command, dtype=np.float64).reshape(3)
        if not (
            np.all(np.isfinite(gravity_body))
        ...
        alpha = float(np.deg2rad(SLOPE_ANGLE_DEG))
        normal, uphill, _ = slope_basis_body(
            gravity_body,
            alpha,
        ...
        com = np.asarray(geometry.com_body, dtype=np.float64).reshape(3)
        tether_start = np.asarray(
            geometry.tether_start_body, dtype=np.float64
        ).reshape(3)
        foot_radii = np.asarray(
            geometry.foot_radii_m, dtype=np.float64
        ).reshape(6)
        if not all(
            np.all(np.isfinite(value))
            for value in (normal, uphill, feet, com, tether_start, foot_radii)
        ):
            return (
                float(self._tether_force_target_n),
                bool(self._tether_controller_feasible),
        ...
        # Contact-free implementation: all six dynamically updated foot sites
        # define the one-dimensional support interval along the uphill axis.
        feet_x = feet @ uphill
        support_min = float(np.min(feet_x))
        support_max = float(np.max(feet_x))
        support_mid = 0.5 * (support_min + support_max)
        w_r = 0.5 * (support_max - support_min)
        w_g = support_mid - float(np.dot(com, uphill))

        support_surface_coord = feet @ normal - foot_radii
        ground_normal_coord = float(np.mean(support_surface_coord))
        h_g = max(
            float(np.dot(com, normal)) - ground_normal_coord,
            1.0e-5,
        )
        h_t = max(
        ...
        weight = ROBOT_MASS_KG * GRAVITY_M_S2
        sin_alpha = float(np.sin(alpha))
        cos_alpha = float(np.cos(alpha))
        mu = FRICTION_COEFFICIENT
        slip_lower = weight * (sin_alpha - mu * cos_alpha)
        slip_upper = weight * (sin_alpha + mu * cos_alpha)

        static_a = weight * (
            h_g * sin_alpha + (-w_r + w_g) * cos_alpha
        ) / h_t
        static_b = weight * (
            h_g * sin_alpha + (w_r + w_g) * cos_alpha
        ) / h_t
        down_a = weight * (
            h_g * sin_alpha + (-w_r - mu * h_g + w_g) * cos_alpha
        ) / h_t
        down_b = weight * (
            h_g * sin_alpha + (w_r - mu * h_g + w_g) * cos_alpha
        ) / h_t
        ...
        static_lower, static_upper = sorted((static_a, static_b))
        down_lower, down_upper = sorted((down_a, down_b))
        up_lower, up_upper = sorted((up_a, up_b))
        commanded_body = np.array(
            [command[0], command[1], 0.0], dtype=np.float64
        )
        commanded_uphill = float(np.dot(commanded_body, uphill))
        if float(np.linalg.norm(command[:2])) <= 1.0e-6:
            tip_lower, tip_upper = static_lower, static_upper
        elif commanded_uphill >= 0.0:
            tip_lower, tip_upper = up_lower, up_upper
        else:
            tip_lower, tip_upper = down_lower, down_upper

        lower = max(MIN_TETHER_FORCE_N, slip_lower, tip_lower)
        upper = min(MAX_TETHER_FORCE_N, slip_upper, tip_upper)
        feasible = bool(lower <= upper)
        nominal = weight * sin_alpha
        if feasible:
            target = float(np.clip(nominal, lower, upper))
        else:
            # Preserve the sim2sim fallback ordering: first retain the
            # turnover-safe interval, then fall back to the actuator interval.
            tip_safe_lower = max(MIN_TETHER_FORCE_N, tip_lower)
            tip_safe_upper = min(MAX_TETHER_FORCE_N, tip_upper)
            if tip_safe_lower <= tip_safe_upper:
                target = float(
                    np.clip(nominal, tip_safe_lower, tip_safe_upper)
                )
            else:
    ...
    def run(self):
        ...
        # Nagatani geometry depends only on the gravity direction.  Validate
        # it independently from the other IMU channels used by the policy so
        # an unrelated velocity/gyro/acceleration fault does not freeze an
        # otherwise valid deterministic tension target.
        nagatani_gravity_body = np.zeros(3, dtype=np.float64)
        gravity_valid = False
        ...
        if (
            ...
            if gravity_norm > 1e-6:
                nagatani_gravity_body = gravity_sample / gravity_norm
                gravity_valid = True
        ...
        if imu_data is None or vel is None or grav is None:
            ...
            if g_norm > 1e-6:
                ...
                # A finite zero vector is still not a usable gravity sample.
                # Make the Nagatani controller hold its preceding target.
                imu_valid = False
        ...
        # Match training: append the deterministic cable ctrl applied during
        # the preceding control interval to the 18 previous leg actions.
        ...
        if self.action.shape != (18,):
            raise ValueError(
                "Nagatani comparison policy must output exactly 18 leg actions, "
                f"got shape {self.action.shape}"
            )

        # The policy controls only the 18 leg joints.  Nagatani, not the
        # policy, supplies the cable target.
        ...
        (
            target_tension,
            tether_controller_feasible,
            tether_geometry_valid,
        ) = self._compute_nagatani_tension(
            gravity_body=nagatani_gravity_body,
            # The joystick returns normalized stick values; training selects
            # the up/down case from the physical command after applying the
            # same per-axis scale used in the policy observation.
            command=self.cmd * self.config.command_scale,
        ...
        # Use the same direct conversion as the constant-force baseline:
        # spool torque tau = tension T * effective radius r.  No tension PID or
        # feedback term is added to this deterministic comparison controller.
        ...
        self._tether_controller_feasible = bool(tether_controller_feasible)
        self._tether_geometry_valid = bool(tether_geometry_valid)
