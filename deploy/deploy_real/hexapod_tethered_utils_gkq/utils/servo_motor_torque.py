            ...
            while time.time() - start_time < duration:
                try:
                    ...
                    if info:
                        ...
                        if isinstance(torque, (int, float)) and torque != 'N/A':
                            torque_ratio = torque / 0.08
                            print(f"{elapsed:.2f}\t{angle_str}°\t{speed_str}\t{torque_str}%\t{torque_ratio:.2f}")
                        else:
                            print(f"{elapsed:.2f}\t{angle_str}°\t{speed_str}\t{torque_str}%\tN/A")
